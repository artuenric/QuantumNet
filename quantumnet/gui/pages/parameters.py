...
from quantumnet.gui.config_io import load_config, save_config
from quantumnet.gui.constants import NOISE_OPTIONS, field_help
from quantumnet.gui.ui import config_selector
from quantumnet.gui.validation import safe_int, safe_probability, validate_config


def _render_decoherence_section(current: dict[str, Any]) -> dict[str, float]:
...
def _render_fidelity_section(current: dict[str, Any]) -> dict[str, float]:
...
def _render_probability_section(current: dict[str, Any]) -> dict[str, float]:
...
def _render_protocol_section(current: dict[str, Any]) -> dict[str, int]:
...
def _render_defaults_section(current: dict[str, Any]) -> dict[str, int | str]:
...
def _render_costs_section(current: dict[str, Any]) -> dict[str, int]:
    st.subheader("Costs")
    return {
...
def render_parameters_page(default_config_path: Path) -> None:
    ...
    with st.form("quantumnet-config-form"):
        decoherence = _render_decoherence_section(current)
        fidelity = _render_fidelity_section(current)
        probability = _render_probability_section(current)
        protocol = _render_protocol_section(current)
        defaults = _render_defaults_section(current)
        costs = _render_costs_section(current)
